...
utils.execute = function execute(serviceCtx, items, props) {
  ...
  let deobfuscateURL =  JSONLDUtilsNp.getV(props.os, PN_P.deobfuscateEndpoint);

  serviceCtx.logger.logJSON('info', { serviceType: serviceCtx.name, action: 'v2Decrypt-Start',
                                      msgId: props.msgId,
                                      os: props.os['@id'],
                                      deobfuscateURL: deobfuscateURL, }, loggingMD);
